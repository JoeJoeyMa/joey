...
Set_config_user(){
	...
	stty erase '^H' && read -p "(默认: Joejoey):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="Joejoey"
...
Set_config_port(){
	...
	stty erase '^H' && read -p "(默认: 366):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="366"
...
Set_config_password(){
	echo "请输入要设置的用户 密码"
	stty erase '^H' && read -p "(默认: Joejoey):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="Joejoey"
...
BUG here	#[[ ! -e ${ssr_folder} ]] && echo -e "${Error} ShadowsocksR服务端 下载失败 !" && exit 1
